...
"Twilio SendGrid" rather than a bare address), how many messages it sent, the
DMARC verdict, and the domain the mail actually **authenticated as**.

That last column is what separates your own misconfigured tooling (which
authenticates as something of yours) from forwarding and from outright
spoofing (which authenticates as a stranger's domain, or not at all).
...
bar to jump to that day's reports. Rendered server-side as SVG, with a table
...
of its required DNS records are actually published. The built-in quarantine
tenant collects mail that no active address claims, so a typo in a DNS record
never means silently losing reports.
...
found, malformed (present but receivers would ignore it), or lookup failed.
...
OIDC provider (Azure AD / Entra, Google Workspace, Keycloak, Okta) and
everyone else signs in through it. Any user can mint personal API tokens,
which are hashed at rest and shown exactly once.
